...
const VSplitter: FunctionComponent<Props> = (props) => {
    const { width, height, initialPosition, onChange, positionFromBottom=false } = props
    ...
    const gripPositionFromTop = positionFromBottom ? height - gripPosition : gripPosition
    ...
    const height1 = gripPositionFromTop - gripThickness / 2 - gripMargin
    ...
    const _handleGripDragStop = (evt: DraggableEvent, ui: DraggableData) => {
        const newGripPositionFromTop = ui.y;
        if (newGripPositionFromTop === gripPositionFromTop) {
            return;
        }
        const newGripPosition = positionFromBottom ? height - newGripPositionFromTop : newGripPositionFromTop
    ...
    return (
        <div className="vsplitter" style={{...style0, position: 'relative'}}>
            ...
            <Draggable
                ...
                key="drag"
                position={{ x: 0, y: gripPositionFromTop - gripThickness / 2 - gripMargin }}
                axis="y"
